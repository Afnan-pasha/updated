...
const MakerNotifications = () => {
  ...
  useEffect(() => {
    const makerNotifications = getNotifications(user.id, 'maker');
    setNotifications(makerNotifications);
  }, [user.id]);
  ...
  const handleNotificationClick = (notification) => {
    ...
    // Makers typically go to the applications list to pick up new apps
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        <Paper elevation={3} sx={{ p: 3, mb: 3, borderRadius: 3, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Button
            ...
            onClick={() => {
              markAllNotificationsRead(user.id, 'maker');
              const updated = getNotifications(user.id, 'maker');
...
export default MakerNotifications;